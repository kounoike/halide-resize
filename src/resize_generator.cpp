...
public:
    ...
    GeneratorParam<bool> upsample_x{"upsample_x", false};
    GeneratorParam<bool> upsample_y{"upsample_y", false};
    ...
    void generate() {
        ...
        // different places.
        Expr inverse_scale_factor_x = strict_float(1.0f / scale_factor_x);
        Expr inverse_scale_factor_y = strict_float(1.0f / scale_factor_y);

        Expr kernel_scaling_x = upsample_x ? Expr(1.0f) : scale_factor_x;
        Expr kernel_scaling_y = upsample_y ? Expr(1.0f) : scale_factor_y;
        Expr inverse_kernel_scaling_x = upsample_x ? Expr(1.0f) : inverse_scale_factor_x;
        Expr inverse_kernel_scaling_y = upsample_y ? Expr(1.0f) : inverse_scale_factor_y;

        Expr kernel_radius_x = 0.5f * kernel_info[interpolation_type].taps * inverse_kernel_scaling_x;
        Expr kernel_radius_y = 0.5f * kernel_info[interpolation_type].taps * inverse_kernel_scaling_y;

        Expr kernel_taps_x = cast<int>(ceil(kernel_info[interpolation_type].taps * inverse_kernel_scaling_x));
        Expr kernel_taps_y = cast<int>(ceil(kernel_info[interpolation_type].taps * inverse_kernel_scaling_y));

        // source[xy] are the (non-integer) coordinates inside the source image
        Expr sourcex = (x + 0.5f) * inverse_scale_factor_x - 0.5f;
        Expr sourcey = (y + 0.5f) * inverse_scale_factor_y - 0.5f;
        ...
        Expr beginx = cast<int>(strict_float(ceil(sourcex - kernel_radius_x)));
        Expr beginy = cast<int>(strict_float(ceil(sourcey - kernel_radius_y)));
        beginx = clamp(beginx, input.dim(0).min(), input.dim(0).max() + 1 - kernel_taps_x);
        beginy = clamp(beginy, input.dim(1).min(), input.dim(1).max() + 1 - kernel_taps_y);

        RDom r_x(0, kernel_taps_x), r_y(0, kernel_taps_y);
        const KernelInfo &info = kernel_info[interpolation_type];

        unnormalized_kernel_x(x, k) = info.kernel((k + beginx - sourcex) * kernel_scaling_x);
        unnormalized_kernel_y(y, k) = info.kernel((k + beginy - sourcey) * kernel_scaling_y);

        kernel_sum_x(x) = sum(unnormalized_kernel_x(x, r_x), "kernel_sum_x");
        kernel_sum_y(y) = sum(unnormalized_kernel_y(y, r_y), "kernel_sum_y");
        ...
        Func resized;
        if (upsample_x) {
            resized_x(x, y, c) = sum(kernel_x(x, r_x) * as_float(r_x + beginx, y, c), "resized_x");
            resized_y(x, y, c) = sum(kernel_y(y, r_y) * resized_x(x, r_y + beginy, c), "resized_y");
            resized = resized_y;
        } else {
            resized_y(x, y, c) = sum(kernel_y(y, r_y) * as_float(x, r_y + beginy, c), "resized_y");
            resized_x(x, y, c) = sum(kernel_x(x, r_x) * resized_y(r_x + beginx, y, c), "resized_x");
            resized = resized_x;
        }
